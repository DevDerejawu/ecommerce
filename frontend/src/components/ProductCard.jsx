  ...
  async function handleAddToCart(product) {
    ...
    try {
      ...
      if (res.data.success) {
        ...
        showPopUpMessage("Unable to add to cart.", "error")
    ...
    <div className="group relative cursor-pointer border rounded-lg overflow-hidden bg-white pb-3">
      ...
      <div className="relative h-64 overflow-hidden">
        ...
        <Link
          ...
          className="absolute cursor-pointer bottom-0 w-full text-center bg-black border px-3 py-3 text-sm opacity-100 rounded md:opacity-0 text-white md:group-hover:opacity-100 transition"
        >
          View Detail
        </Link>
      </div>
